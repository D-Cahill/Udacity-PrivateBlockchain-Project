...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                //Check if elapsed time is less than 5 mins
                if((messageTime - currentTime) < 300){
                    ...
                    if(bitcoinMessage.verify(message, address, signature)){
                        ...
                        resolve(block);
                    } else { //Invalid message verification
                        reject(new Error('Invalid message verification.'));
                    }
                } else { // If more than 5 minutes.
                    reject(new Error('More than five minutes has elapsed.'));
                }
            } catch (err) {
                reject(new Error(err));
            }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            try{
                //Find block by hash 
                const resultBlock = this.chain.find((block) => hash === block.hash);
                //Resolve with found block
                resolve(resultBlock);
            } catch (err) {
                reject(new Error(err));
            }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                self.chain.forEach(async(block) => {
                    ...
                    //Check if star owner equals address param
                    if (data.address === address){
                        stars.push(data); // pushes data to stars array.
                    }
                });
                //Resolve with stars array.
                resolve(stars);
            } catch (err) {
                reject(new Error(err));
            }
